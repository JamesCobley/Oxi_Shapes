...
@functor GenGeoAlpha  # ← this makes θ_geo and θ_flow trainable
...
function train_geo_flow_model(config::GeoFlowConfig)
  # Initialize
  model = GenGeoAlpha(ones(Float32, length(config.pf_states)), [1.0f0])
  opt   = Adam(1e-4)
  ps    = Flux.params(model)    # now picks up θ_geo & θ_flow
  ...
  for epoch in 1:config.epochs
    ...
    for _ in 1:config.batch_size
      ...
      # Compute gradients over **all** parameters in `ps`
      gs = gradient(ps) do
        geo_flow_rollout_loss(ρ0, model, config; T=config.rollout_steps)
      end
