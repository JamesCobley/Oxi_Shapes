...
function generate_empirical_k_constrained_initials(num_samples::Int, k_distribution::Dict{Int, Int}, pf_states::Vector{String}; total_mass::Int = 100)
    grouped = Dict(k => filter(s -> count(==('1'), s) == k, pf_states) for k in keys(k_distribution))
    samples = Vector{NamedTuple{(:uuid, :counts, :rho), Tuple{UUID, Vector{Int}, Vector{Float32}}}}()

    for _ in 1:num_samples
        counts = zeros(Int, length(pf_states))
        for (k, group_count) in k_distribution
            chosen_state = rand(grouped[k])  # choose randomly within group
            idx = findfirst(==(chosen_state), pf_states)
            counts[idx] += group_count
        end
        ρ = Float32.(counts) ./ sum(counts)
        push!(samples, (uuid=uuid4(), counts=counts, rho=ρ))
    end

    batch_id = "empirical_$(Dates.format(now(), "yyyymmdd_HHMMSS"))"
...
# --- Parameters ---
num_initials = 1000          # Number of initial distributions
total_molecules = 100       # Total molecules per distribution
simulation_steps = 240      # Number of time steps per simulation

# --- Generate and Save Initials ---
# Define empirical start priors
k_start_distribution = Dict(0 => 25, 1 => 75)

batch_id, initials = generate_empirical_k_constrained_initials(1000, k_start_distribution, pf_states)
