...
# One step of the flow: returns (new_state, λ)
function step(
    state::ComplexField,
    model::GenGeoAlpha,
    config::GeoFlowConfig
)::Tuple{ComplexField, Float32}
    # 1) Copy & evolve real (mutations black-boxed by rrule)
    ...
    # 4) Update imaginary field purely
    rawW = exp.(state.memory .+ C_R_scaled .- maximum(state.memory .+ C_R_scaled))
    W    = rawW ./ sum(rawW)
    rawI = (1f0 - λ) .* state.imag .+ λ .* W
    imag′ = max.(rawI, 0.0f0) ./ sum(max.(rawI, 0.0f0))

    # 5) Update memory purely (avoid in-place)
    tmpMem = (state.memory .+ real′) ./ 2f0
    mem′   = tmpMem ./ sum(tmpMem)

    return ComplexField(real′, imag′, mem′), λ
end

# Compute loss = mean λ over T steps, no array mutations
function geo_flow_rollout_loss(
    ...
    state = ComplexField(real=ρ0, imag=zeros(Float32, length(ρ0)), memory=copy(ρ0))

    # Accumulate λ in a scalar
    total_λ = 0f0

    # Loop without array broadcasts
    for _ in 1:T
        state, λ = step(state, model, config)
        total_λ += λ
    end
...
function train_geo_flow_model(config::GeoFlowConfig)
    ...
    for epoch in 1:config.epochs
        ...
        for _ in 1:config.batch_size
            ...
            # 2) Compute ∂(mean λ)/∂θ_geo and ∂(mean λ)/∂θ_flow
            grads_geo, grads_flow = gradient(
              (θg, θf) -> geo_flow_rollout_loss(
                             ρ0,
                             GenGeoAlpha(θ_geo=θg, θ_flow=θf),
                             config;
                             T=config.rollout_steps
                         ),
              model.θ_geo, model.θ_flow
            )

            # 3) Apply a small manual update to your parameters
            η_geo, η_flow = 1e-3f0, 1e-3f0
            model.θ_geo  .-= η_geo  .* grads_geo
            model.θ_flow .-= η_flow .* grads_flow

            # 4) Recompute λ under updated model for logging
        ...
        push!(λ_epoch_history, mean(λ_vals))
        println("Epoch $epoch | λ (mean) = $(round(mean(λ_vals), digits=6))")
    end
