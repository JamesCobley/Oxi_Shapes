...
# ---------- Build modal Laplacian with curvature fusion ----------
function diamond_from_curved(w_energy::Vector{Float64}, κ_nodes::Vector{Float64};
                             mode::Symbol = :geometric)
    wbits = similar(w_energy)
    if mode == :geometric
        @inbounds for i in eachindex(wbits)
            wbits[i] = w_energy[i] * (1 + κ_nodes[i])
        end
    elseif mode == :tropical
        @inbounds for i in eachindex(wbits)
            cE = -log(w_energy[i] + eps())
            cK = -log(κ_nodes[i] + eps())
            wbits[i] = exp( -min(cE, cK) )
        end
    end
    wbits ./= sum(wbits)
    return diamond_from_weights(wbits), wbits
...
((WdC,LdC), w_curved) = diamond_from_curved(w_energy, κ_nodes; mode=:geometric)
